 ...
 *
 */
public class Caleb extends Player
{
	
	public Caleb(String n, int t, int l)
	{
		super(n, t, l);
	}
	...
	public Move getMove(StateTree state)
	{
		...
		// using a two-condition tapered search
		int maxDepth = (state.columns > 7? 3 : 7);
		...
		for (int i = 0; i < state.children.size(); i++) {
			...
			moveValues[i] = (state.turn == 1 ? getAlphaMax(state.children.get(i), maxDepth, 1, Integer.MIN_VALUE, Integer.MAX_VALUE) : 
				getAlphaMin(state.children.get(i), maxDepth, 1, Integer.MIN_VALUE, Integer.MAX_VALUE));
		}
	...
	public int getMax(StateTree state, int m, int d) {
		if (m <= d) {
			...
			state.children = populateChildren(state);
			for (int i = 0; i < state.children.size(); i++) {
				int value = getMin(state.children.get(i), m, d + 1);
	...
	// minimax-ab-pruning max function
	public int getAlphaMax(StateTree state, int m, int d, int a, int b) {
		if (m <= d) {
			return evalBoard(state);
		}
		else {
	...
	public int getMin(StateTree state, int m, int d) {
		if (m <= d) {
			return evalBoard(state);
		}
		else {
			...
			state.children = populateChildren(state);
			for (int i = 0; i < state.children.size(); i++) {
				int value = getMax(state.children.get(i), m, d + 1);
	...
	// minimax-ab-pruning min function
	public int getAlphaMin(StateTree state, int m, int d, int a, int b) {
		if (m <= d) {
			return evalBoard(state);
		}
		else {
	...
	// helper function to get available moves given a board state 
	...
	// helper function to identify which cells in boardMatrix to evaluate
	...
	// helper function to find relevant cells for a vertical connect-n
	...
	// helper function to find relevant cells for a horizontal connect-n
	...
	// helper function to find relevant cells for a upper diagonal connect-n
	...
	// helper function to find relevant cells for a lower diagonal connect-n
